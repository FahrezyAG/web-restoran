...
$queryjumlah = mysqli_query($conn, "SELECT * FROM minuman");
$jumlahminuman = mysqli_num_rows($queryjumlah);
      ...
      <table class="min-w-full dark:bg-[#2b493B]shadow-md dark:bg-[#1E293B] rounded-lg ">
        <thead>
          <tr class="bg-blue-gray-100 ">
            ...
            <th class="py-3 px-4 text-left">Minuman</th>
            <th class="py-3 px-4 text-left">Harga</th>
            <th class="py-3 px-4 text-left">Gambar</th>
            <th class="py-3 text-left px-6">Aksi</th>
          </tr>
        </thead>
          ...
          while ($data = mysqli_fetch_array($queryjumlah)) {
            if ($jumlahminuman == 0) {
              ...
              <tr class="border-b border-blue-gray-200">
                ...
                <td class="py-3 px-4"><?php echo $data['nama_minuman'] ?></td>
                <td class="py-3 px-4"><?php echo $data['harga'] ?></td>
                <td class="py-3 px-4 max-w-1">
                  <?php echo '<img src="data:image/jpeg;base64,' . base64_encode($data['image']) . '"/>'; ?>
                </td>
                <td class="py-3 px-4">
                <a href="./proses3.php?id=<?php echo $data['id']; ?>"
